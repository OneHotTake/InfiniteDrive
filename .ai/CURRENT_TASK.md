...
COMPLETE: AIOStreams password fixes per user feedback. (1) NEVER PERSIST: removed PluginConfiguration.PrimaryManifestPassword + load/save of it; field blank on load, Save doesn't persist (verified: 0 in config after Save), scrubbed leftover empty element, copy now "Used once, in-memory…never saved." (2) "Doesn't work": plugin actually works (Preview diff + Apply changed instance, both verified live with real pw "May30th-"); root cause of user's error = flaky/slow instance (intermittent 502s, ~9-12s) — added robustness: AioStreamsConfigClient timeout 20s→45s + explicit 502/503/504 ("temporarily unavailable, try again") and timeout ("slow or unreachable") messages. NOTE: page has 2 password inputs — automation must target the AIOStreams field specifically. 44/44 tests pass.
